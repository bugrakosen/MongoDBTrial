using MongoDBTrial.Helpers.ExpandoLib.Utilities;
using System;
    ...
    public class PropertyBag<TValue> : Dictionary<string, TValue>, IXmlSerializable
    {
        ...
        public void WriteXml(System.Xml.XmlWriter writer)
        {
            foreach (string key in this.Keys)
            {
                ...
                string xmlType = Utilities.Utilities.MapTypeToXmlType(type);
                bool isCustom = false;
            ...
            while (reader.Read())
            {
                if (reader.NodeType == XmlNodeType.Element && reader.Name == "key")
                {
                    ...
                    else if (xmlType.StartsWith("___"))
                    {
                        ...
                        Type type = Utilities.Utilities.GetTypeFromName(xmlType.Substring(3));
                        XmlSerializer ser = new XmlSerializer(type);
                        value = (TValue)ser.Deserialize(reader);
                    }
                    else
                        value = (TValue)reader.ReadElementContentAs(Utilities.Utilities.MapXmlTypeToType(xmlType), null);
